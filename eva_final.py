...
class SimpleDataset(torch.utils.data.Dataset):
    def __init__(self, files, height, width):
        self.files = files
        # 预处理：调整大小并转为Tensor
    ...
    def __getitem__(self, idx):
        ...
        # 构造原代码需要的字典格式 key: ("color", 0, 0)
        return {("color", 0, 0): self.transform(img)}
...
def evaluate(opt):
    ...
    if opt.ext_disp_to_eval is None:
        ...
        # --- 2. 获取文件列表并强制排序 ---
        # sorted() 是保证 rgb 和 gt 严格对应的关键
        rgb_files = sorted(glob.glob(os.path.join(rgb_dir, "*.jpg")))
        gt_files = sorted(glob.glob(os.path.join(gt_dir, "*.png")))

        # --- 3. 安全检查 ---
        assert len(rgb_files) > 0, "No images found!"
        assert len(rgb_files) == len(gt_files), f"Mismatch: {len(rgb_files)} JPGs vs {len(gt_files)} PNGs"
        # 检查首个文件名是否匹配 (去后缀)
        assert os.path.splitext(os.path.basename(rgb_files[0]))[0] == \
               os.path.splitext(os.path.basename(gt_files[0]))[0], "Filename mismatch!"

        print(f"-> Found {len(rgb_files)} images. Evaluating...")

        # --- 4. 实例化 Dataset 和 DataLoader ---
        dataset = SimpleDataset(rgb_files, encoder_dict['height'], encoder_dict['width'])
    ...
    print(f"-> Using GT from folder: {gt_dir}")
    ...
    for i in range(pred_disps.shape[0]):
        gt_depth = cv2.imread(gt_files[i], -1)
        gt_depth = gt_depth.astype(np.float32) / 1000.0
        gt_height, gt_width = gt_depth.shape[:2]
        pred_disp = pred_disps[i]
        pred_disp = cv2.resize(pred_disp, (gt_width, gt_height))
        pred_depth = 1 / pred_disp
        mask = np.logical_and(gt_depth >= 1.0, gt_depth <= 1.2)
